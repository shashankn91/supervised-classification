...
class LargeAlgoEval:
    ...
    def evalFinal(self,X_train,Y_train,X_test,Y_test):
        ...
        ann = MLPClassifier(activation='relu' ,solver='lbfgs', alpha=1e-5,
                       random_state=1,hidden_layer_sizes = (20,18,16,14))
        ...
        linSVM = LinearSVC(intercept_scaling = 10000,tol=1e-6 , dual=False , C = 2)
        ...
        # Decision Tree
        dtClf = tree.DecisionTreeClassifier(max_depth =12)
    ...
    def evalLearningCurve(self,X_train,Y_train,folderName):
        ...
        self.plot_learning_curve(estimator=ann,title="ANN Learning Curve",X=X_train,y=Y_train,ylim=None,cv =10,n_jobs=1)
        plt.savefig(folderName + '/ANNLearningCurve.png')
        print("ANN Done")

        #SVM
        linSVM = LinearSVC(intercept_scaling = 10000,tol=1e-6 , dual=False , C = 2)
        self.plot_learning_curve(estimator=linSVM,title="SVM Learning Curve",X=X_train,y=Y_train,ylim=None,cv =10,n_jobs=1)
        plt.savefig(folderName + '/SVMLearningCurve.png')
        print("SVM Done")

        #kNN
        neigh = KNeighborsClassifier(n_neighbors = 12)
        self.plot_learning_curve(estimator=neigh,title="KNN Learning Curve",X=X_train,y=Y_train,ylim=None,cv =10,n_jobs=10)
        plt.savefig(folderName + '/KNNLearningCurve.png')

        #AdaBoost
        adaClf = AdaBoostClassifier(n_estimators = 16,
                                    learning_rate=1,
                                    random_state=0)
        self.plot_learning_curve(estimator=adaClf,title="AdaBoost Learning Curve",X=X_train,y=Y_train,ylim=None,cv =10,n_jobs=10)
        plt.savefig(folderName + '/AdaBoostLearningCurve.png')

        # Decision Tree
        dtClf = tree.DecisionTreeClassifier(max_depth =12)
        self.plot_learning_curve(estimator=dtClf,title="Decision Tree Learning Curve",X=X_train,y=Y_train,ylim=None,cv =10,n_jobs=10)
    ...
    def evalAllValidationCurves(self,X_train,Y_train,folderName):
        ...
        ann = MLPClassifier(activation='relu' ,solver='lbfgs', alpha=1e-5,
                       random_state=1)
        self.plot_validation_curve(ann,"ANN Validation Curve",X_train,Y_train,"hidden_layer_sizes",[ (14),(16,14),(18,16,14),(20,18,16,14),(22,20,18,16,14),(24,22,20,18,16,14)],cv =4,n_jobs=1)
        ...
        self.plot_validation_curve(linSVM,"SVM Validation Curve",X_train,Y_train,"C",[0.0001,0.001,0.01,0.1,1,2,4,16,32,64,256],cv =10,n_jobs=1)
        plt.savefig(folderName + '/SVMValidationCurve.png')
        print("SVM Done")

        #kNN
        neigh = KNeighborsClassifier()
        self.plot_validation_curve(neigh,"KNN Validation Curve",X_train,Y_train,"n_neighbors",[1,2,4,6,8,12,14,16,64],cv =10,n_jobs=10)
        ...
        self.plot_validation_curve(adaClf,"AdaBoost Validation Curve",X_train,Y_train,"n_estimators",range(1,50,3),cv =10,n_jobs=50)
        plt.savefig(folderName + '/AdaBoostValidationCurve.png')

        # Decision Tree
        dtClf = tree.DecisionTreeClassifier()
        self.plot_validation_curve(dtClf,"Decision Tree Validation Curve",X_train,Y_train,"max_depth",[1,2,4,6,8,12,14,16,32],cv =10,n_jobs=10)
